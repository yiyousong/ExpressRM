...
parser.add_argument('--seq', default=True, help='use 1/0 for True/False')
parser.add_argument('--gene', default=False, help='use 1/0 for True/False')
parser.add_argument('--genelocexp', default=False, help='use 1/0 for True/False')
parser.add_argument('--geo', default=False, help='use 1/0 for True/False')
parser.add_argument('--tgeo', default=False, help='use 1/0 for True/False')
parser.add_argument('--featurelist', default=None, help='ignore this. 1,0,0,0,0\n sequence,gene,genelocexp,geo,tgeo')
...
parser.add_argument('--prefix', default='', help='prefix for models')
parser.add_argument('--autoprefix', default=True, help='autoprefix for models')
parser.add_argument('--trainlist', default=None, help='ignore this. use testlist')
...
def weightlabel(labelwithsite):
    ### the labels generated by step2 contains the transcriptome label and sitelabel at the same time, this needs to be seperated
    ### [N,38]-> [N,37],[N,37],[N,37]
...
class ExpressRM(pl.LightningModule):
    ...
    def forward(self, x, geo, gene, genelocexp):
        ### x(sequence) size [N, 4, sequence length] one-hot encoded sequence
        ### geo(log transformed) [N, 37?, 12] geographic encoding across 37 transcriptomes across sites
        ### gene [37?, 28278] gene expression across 37 transcriptomes
        ### genelocexp [N, 37?, 1] hosting gene expression across 37 transcriptomes across sites
        batchsize = x.size()[0]
    ...
    def shared_step(self, batch):
        ...
        x = x.flatten(0,1)
        geo = geo.flatten(0,1)
        genelocexp = genelocexp.flatten(0,1).unsqueeze(2)
        pred = self.forward(x, geo, gene, genelocexp)
        pred=pred.view([-1,4])
